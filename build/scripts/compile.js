...
const compile = () => Promise.resolve()
  ...
  .then((stats) => {
    if (project.publicFolder) {
      logger.info(`Copying static assets from ./${project.publicFolder} to ./${project.outDir}.`)
      if (!fs.existsSync(path.resolve(project.basePath, 'public'))) {
        logger.info(`./${project.publicFolder} to ./${project.outDir}.`)
      }
      fs.copySync(
        path.resolve(project.basePath, 'public'),
        path.resolve(project.basePath, project.publicFolder)
      )
    }
    return stats
  })
